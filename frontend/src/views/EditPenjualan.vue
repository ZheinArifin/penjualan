      ...
      <div class="row">
        <div class="col">
            <div class="card p-5">
                <div class="field mb-3">
                    ...
                    <div class="control">
                        <select name="barang_id" required v-model="barang" @change="onChangeChain" class="form-control" style="color:black">
                          ...
                          <option  style="color:black" v-for="link in barangs" :value="link.id" :key="link.id" selected>{{link.nama_barang}}</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="control">
                    <button class="btn btn-success" @click="updatePenjualan">UPDATE</button>
...
export default {
  ...
  methods: {
    ...
    // Get Product By Id
    async getPenjualanById() {
      try {
        const response = await axios.get(
          `http://localhost:5000/penjualan/${this.$route.params.id}`
        );
        this.barang = response.data.id_barang;
    ...
    async updatePenjualan() {
      try {
        await axios.put(`http://localhost:5000/penjualan/${this.$route.params.id}`,
          {
        ...
        this.$swal({
            title:"Success!",
            text:"Berhasil Merubah Data Penjualan!",
            icon:"success",
            button:false,
            timer:3000
        }).then(function(){
